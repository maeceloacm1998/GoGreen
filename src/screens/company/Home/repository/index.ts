...
export async function getSchedulingListCompany(companyId: string): Promise<Array<ScheduleModel>> {
  const request: Array<ScheduleModel> = await httpClient.get(SCHEDULING_LIST_PATH);
  return request.filter((item) => item.idCompany.toString() == companyId);
}
